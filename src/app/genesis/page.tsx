...
export default function GenesisPortal() {
  ...
  return (
    <div className="h-full m-4 flex flex-col relative overflow-hidden">
      ...
      <div className="flex-1 flex gap-6 px-8 overflow-hidden">
        ...
        <div className="w-1/3 flex flex-col gap-4">
          <div className="glass-panel p-6 flex flex-col flex-1 relative shimmer-hover">
            ...
            <textarea 
              className="flex-1 w-full bg-slate-100 border border-slate-300 rounded-xl p-4 text-slate-900 placeholder-slate-500 focus:outline-none focus:border-blue-500/50 transition-colors resize-none"
              placeholder="Paste a Jira URL, type a raw idea, or paste an API spec here..."
              value={input}
              onChange={(e) => setInput(e.target.value)}
            />
            
            <div className="mt-4 p-4 border border-dashed border-slate-300 rounded-xl bg-slate-50 text-center flex flex-col items-center justify-center cursor-pointer hover:border-blue-500/50 transition-colors">
              <svg className="w-8 h-8 text-slate-800 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
              <p className="text-sm text-slate-800 font-medium">Drag & Drop Image or PDF Spec</p>
              <p className="text-xs text-slate-800 mt-1">Multi-modal analysis powered by Gemini 2.0</p>
            </div>
        ...
        <div className="w-2/3 glass-panel p-6 flex flex-col">
          ...
          {!result && !loading && (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-800 border border-dashed border-slate-300 rounded-xl bg-slate-50">
              <div className="w-16 h-16 rounded-full bg-slate-800/50 flex items-center justify-center mb-4">
                <svg className="w-8 h-8 text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
              </div>
          ...
          {loading && (
            <div className="flex-1 flex flex-col items-center justify-center text-blue-400 border border-slate-300 rounded-xl bg-slate-100">
              <div className="relative w-24 h-24 mb-6">
                ...
                <div className="absolute inset-2 rounded-full border-b-2 border-violet-400 animate-spin flex items-center justify-center">
                  <svg className="w-6 h-6 text-slate-900 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                </div>
              </div>
          ...
          {result && (
            <div className="flex-1 flex flex-col overflow-hidden animate-slide-up">
              <div className="flex gap-2 border-b border-slate-300 pb-4 mb-4 flex-wrap">
              ...
              <div className="flex-1 bg-\[#ffffff\] rounded-xl overflow-hidden relative">
                <div className="absolute top-0 left-0 right-0 h-8 bg-\[#e2e8f0\] flex items-center px-4 border-b border-slate-300">
                  <div className="flex gap-1.5">
                    <div className="w-3 h-3 rounded-full bg-red-500"></div>
                    <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                    <div className="w-3 h-3 rounded-full bg-green-500"></div>
                  </div>
                  <span className="text-xs text-slate-800 font-mono ml-4">generated_output.{activeTab === 'qaTests' ? 'spec.ts' : activeTab === 'devOpsConfig' ? 'yml' : activeTab === 'backendSql' ? 'sql' : activeTab === 'apiSpecs' ? 'yaml' : activeTab === 'mobileCode' ? 'tsx' : activeTab === 'productDocs' || activeTab === 'infraCost' ? 'md' : 'tsx'}</span>
                </div>
                <pre className="p-4 pt-12 overflow-auto h-full text-sm font-mono text-slate-800">
              ...
              <div className="mt-4 p-4 rounded-xl border border-dashed border-blue-500/30 bg-blue-500/5 flex items-center justify-between animate-slide-up delay-300 flex-shrink-0">
                ...
                <div className="flex gap-2">
                  <Link href="/sandboxes/gen-ai" title="Best for validating BDD Docs & SQL" className="px-3 py-1.5 rounded-lg bg-slate-100 border border-blue-500/20 hover:border-blue-400 hover:bg-blue-500/10 text-xs text-slate-800 transition-all">
                    Evaluate Logic
                  </Link>
                  <Link href="/sandboxes/ai-agent" title="Best for validating API Specs & DevOps" className="px-3 py-1.5 rounded-lg bg-slate-100 border border-violet-500/20 hover:border-violet-400 hover:bg-violet-500/10 text-xs text-slate-800 transition-all">
                    Simulate APIs
                  </Link>
                  <Link href="/sandboxes/agentic-arena" title="Best for running Playwright & React UI" className="px-3 py-1.5 rounded-lg bg-slate-100 border border-rose-500/20 hover:border-rose-400 hover:bg-rose-500/10 text-xs text-slate-800 transition-all flex items-center gap-1">
  ...
  return (
    <button 
      ...
      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2
        ${active ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-transparent text-slate-800 hover:bg-slate-200/50 border border-transparent'}`}
    >
