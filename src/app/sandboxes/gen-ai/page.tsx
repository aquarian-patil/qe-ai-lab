...
export default function GenAiEvaluator() {
  ...
  return (
    <div className="h-full m-0 md:m-4 flex flex-col relative overflow-hidden glass-panel !border-0 md:!border !rounded-none md:!rounded-2xl">
      <header className="px-4 md:px-8 py-6 border-b border-slate-700 bg-slate-950/50 shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Validate LLM outputs for structural soundness, hallucinations, and prompt injections.</p>
        </div>
        <div className="flex items-center gap-3 bg-slate-900 border border-slate-700 rounded-lg p-1.5 shadow-sm">
          <span className="text-xs font-bold text-slate-400 px-2 uppercase">Model Target</span>
          <select 
            value={model} 
            onChange={(e) => setModel(e.target.value)}
            className="bg-slate-950 border border-slate-700 text-slate-300 text-sm rounded-md focus:ring-teal-500 focus:border-teal-500 block p-2 outline-none font-semibold cursor-pointer"
          >
          ...
          <div className="flex-1 border border-slate-700 rounded-xl bg-slate-900 shadow-sm p-6 flex flex-col">
            ...
            <div className="flex flex-col gap-6 flex-1">
              <div className="flex flex-col shrink-0">
                ...
                <textarea 
                  className="w-full bg-slate-950 border border-slate-700 shadow-[inset_0_1px_2px_rgba(0,0,0,0.02)] rounded-xl p-4 text-slate-300 text-sm leading-relaxed focus:bg-white focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 transition-all resize-none outline-none"
                  rows={3}
        ...
        <div className="w-full lg:w-[45%] bg-slate-950 rounded-xl shadow-2xl border border-slate-800 p-6 flex flex-col shrink-0 lg:h-full relative min-h-[400px] min-w-0">
          ...
          {!result && !evaluating ? (
            <div className="flex-1 flex flex-col items-center justify-center opacity-30">
              <svg className="w-16 h-16 text-slate-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
