    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <header className="h-16 glass-panel mx-4 mt-4 px-6 flex justify-between items-center z-30 relative shrink-0">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <button 
            className="md:hidden p-2 text-slate-800 hover:bg-slate-200/50 rounded-lg transition-colors"
          ...
          <ComingSoon>
            <button className="relative text-slate-800 hover:text-blue-600 transition-colors hidden md:block">
          ...
          <ComingSoon>
            <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-300 overflow-hidden hover:opacity-80 transition-opacity hidden md:block">
      ...
      <aside className={`
        ...
        border-r border-slate-200 shadow-2xl md:shadow-none
        transition-transform duration-300 ease-in-out
        ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        <div className="md:hidden flex justify-between items-center mb-6 border-b border-slate-200 pb-4">
          <h2 className="font-bold text-slate-800">Menu</h2>
          <button onClick={() => setMobileMenuOpen(false)} className="p-1 hover:bg-slate-200 rounded-full text-slate-500">
        ...
        <Link href="/genesis" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-transparent text-slate-800 hover:text-blue-600 hover:bg-slate-200/50 transition-colors group relative overflow-hidden mb-6">
        ...
        <nav className="space-y-1 mb-6">
          <Link href="/approvals" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-slate-800 hover:bg-slate-200/50 hover:text-blue-600 rounded-lg transition-colors group cursor-pointer">
            <span className="text-xl group-hover:scale-110 transition-transform">🛡️</span>
            HITL Approvals
            <HitlBadge />
          </Link>
          <Link href="/audit" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-slate-800 hover:bg-slate-200/50 hover:text-blue-600 rounded-lg transition-colors group cursor-pointer">
            <span className="text-xl group-hover:scale-110 transition-transform">📜</span>
            Audit Logs
          </Link>
          <Link href="/integrations" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-slate-800 hover:bg-slate-200/50 hover:text-blue-600 rounded-lg transition-colors group cursor-pointer">
            <span className="text-xl group-hover:scale-110 transition-transform">🔌</span>
            Integrations Config
          </Link>
        </nav>
...
function NavItem({ href, label, icon, onClick }: { href: string; label: string; icon: string; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300 text-slate-800 hover:text-blue-600 hover:bg-slate-200/50`}>
